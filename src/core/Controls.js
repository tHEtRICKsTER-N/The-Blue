...
export function normalizeControls(value){
  ...
  // Accept a complete, conflict-free map atomically; damaged maps retain safe defaults.
  const seen={dive:new Set(),photo:new Set()},candidate={};
  for(const [id,,group] of actions){
    const keys=value.bindings?.[id];
...
export function loadControls(storage){try{return {settings:normalizeControls(JSON.parse(storage.getItem(CONTROL_STORAGE_KEY))),saved:true};}catch{return {settings:defaultControls(),saved:false};}}
