...
export function ControlSettings({value,onChange,saved}:{value:ControlPreferences;onChange:(next:ControlPreferences)=>void;saved:boolean}){
  ...
  return <div className="control-settings">
    ...
    <div className="set-row"><div className="set-label"><span>Look sensitivity</span><small>Mouse and keyboard look speed while swimming</small></div><div className="set-control"><Slider className="set-slider" aria-label="Look sensitivity" min={.2} max={3} step={.1} value={[value.sensitivity]} onValueChange={v=>onChange({...value,sensitivity:Array.isArray(v)?v[0]:v})}/><em>{value.sensitivity.toFixed(1)}×</em></div></div>
    ...
    <div className="set-row"><div className="set-label"><span>Reduced camera motion</span><small>Remove camera sway, swimming bob and menu entrance animations</small></div><Switch aria-label="Reduced camera motion" checked={value.reducedMotion} onCheckedChange={reducedMotion=>onChange({...value,reducedMotion})}/></div>
    <p className="binding-help">Select a key, then press its replacement. Esc cancels. Keys use physical keyboard positions; Ctrl and Shift work on either side. Arrow keys look around by default. Tab opens the dive menu. Esc always pauses or leaves photo mode.</p>
