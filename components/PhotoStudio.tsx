...
type Props={controls:ControlPreferences;onClose:()=>void;capture:(aspect:string)=>Promise<Photo>;aim:(x:number,y:number)=>void;zoom:(fov:number)=>void;initialFov:number;album:PhotoAlbum;notes:FieldNote[]};

export function PhotoStudio({controls,onClose,capture,aim,zoom,initialFov,album,notes}:Props) {
  const [aspect,setAspect]=useState('original'),[fov,setFov]=useState(initialFov),[grid,setGrid]=useState(true);
  ...
  return <section className="photo-studio" aria-label="Underwater photography">
    ...
    <div className="photo-console">
      <fieldset className="photo-frames"><legend>Frame</legend>{[['original','Original'],['wide','16:9'],['square','Square']].map(([value,label])=><Button key={value} variant="outline" aria-pressed={aspect===value} onClick={()=>setAspect(value)}>{label}</Button>)}</fieldset>
      ...
      <div className="photo-zoom"><span>Lens · {Math.round(fov)}°</span><Slider aria-label="Photo lens field of view" min={25} max={100} step={1} value={[fov]} onValueChange={value=>{const next=Array.isArray(value)?value[0]:value;setFov(next);zoom(next);}}/></div>
      <label className="photo-grid-toggle" htmlFor="photo-grid">Framing grid<Switch id="photo-grid" checked={grid} onCheckedChange={setGrid} aria-label="Framing grid"/></label>
